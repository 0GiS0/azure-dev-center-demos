...
resource "azurerm_dev_center_project" "accounting" {
  name                = "accounting"
  dev_center_id       = azurerm_dev_center.default.id
  resource_group_name = azurerm_resource_group.default.name
  location            = azurerm_resource_group.default.location
...
resource "azurerm_dev_center_project_pool" "accounting-default" {
  for_each = azurerm_dev_center_dev_box_definition.default

  name                                    = "${azurerm_dev_center_project.accounting.name}-${each.value.name}-default-pool"
  location                                = azurerm_dev_center_project.accounting.location
...
resource "azurerm_dev_center_project" "capital-markets" {
...
resource "azurerm_dev_center_project_pool" "capital-markets-default" {
  for_each = azurerm_dev_center_dev_box_definition.default

  name                                    = "${azurerm_dev_center_project.capital-markets.name}-${each.value.name}-default-pool"
  location                                = azurerm_dev_center_project.capital-markets.location
  dev_center_project_id                   = azurerm_dev_center_project.capital-markets.id
  dev_box_definition_name                 = each.value.name
  local_administrator_enabled             = true
  dev_center_attached_network_name        = azurerm_dev_center_attached_network.default.name
  stop_on_disconnect_grace_period_minutes = 60
